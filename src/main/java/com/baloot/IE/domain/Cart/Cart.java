...
//    private static final AtomicInteger count = new AtomicInteger(0);
    ...
    public void add(Product p) throws Exception {
        ...
        if(cartItem == null)
            ...
            buyListRepository.update("quantity", "1", String.valueOf(cartItem.getProduct().getId()), new ArrayList<>(Arrays.asList(String.valueOf(cartItem.getProduct().getId()), String.valueOf(cartItem.getCartId()))));
    }
    ...
    public void remove(Product p) throws SQLException {
        ...
        if(ci != null) {
            ...
            if(ci.isOut())
                ...
                buyListRepository.update("quantity", "-1", String.valueOf(ci.getProduct().getId()), new ArrayList<>(Arrays.asList(String.valueOf(ci.getProduct().getId()), String.valueOf(ci.getCartId()))));
            total -= p.getPrice();
            cartRepository.update("total", String.valueOf(total), "username", username);
            no_items -= 1;
            cartRepository.update("no_items", String.valueOf(no_items), "username", username);
